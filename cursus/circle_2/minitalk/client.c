...
void send_sig(int server_pid, char *message, siginfo_t *siginfo)
{
    static int  bit;
    static int  byte;
    int         temp;
...
void get_sig(int signal, siginfo_t *siginfo, void *context)
{
    if (signal == SIGUSR1) // server에서 signal을 성공적으로 받았을 때
    {
        ...
        send_sig(g_cli.server_pid, g_cli.message, siginfo);
    }
...
void    initial_connect(siginfo_t *siginfo)
{
